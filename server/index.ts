...
(async () => {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    app.post("/api/voice-recordings/process-direct", authenticateImmediate, async (req: any, res) => {
      try {
        console.log("🎙️ HEROKU PRIORITY: Voice processing started with affinity fix");
        ...
        console.log("🎙️ HEROKU PRIORITY: Processing:", finalTranscript.substring(0, 50));

        // Import modules with timeout protection
        const modules = await Promise.race([
          Promise.all([
            import('./lib/ai-processor'),
            import('./storage'),
            import('./lib/affinity-matcher')
          ]),
          new Promise((_, reject) => setTimeout(() => reject(new Error('Module import timeout')), 10000))
        ]);

        const [aiProcessor, storageModule, affinityModule] = modules;
        const { extractInformationFromTranscript, enhanceInteractionComments, assessInteractionQuality } = aiProcessor;
        const { storage } = storageModule;
        const { createAffinityMatcher } = affinityModule;

        // AI extraction with timeout
        console.log("🤖 HEROKU PRIORITY: Starting AI extraction...");
        const userId = req.user?.claims?.sub || req.user?.id || "42195145";
        const extractedInfo = await Promise.race([
          extractInformationFromTranscript(finalTranscript, userId),
          new Promise((_, reject) => setTimeout(() => reject(new Error('AI extraction timeout')), 20000))
        ]);
        console.log("🤖 HEROKU PRIORITY: AI extraction completed");

        // FORCE AFFINITY MATCHING - Bypass any route-specific issues
        let suggestedAffinityTags: string[] = [];
        console.log("🎯 HEROKU PRIORITY: FORCING affinity matching execution...");
        
        try {
          const affinityTags = await storage.getAffinityTags();
          console.log("🎯 HEROKU PRIORITY: Loaded affinity tags:", affinityTags.length);
          ...
          if (affinityTags.length > 0) {
            ...
            console.log("🎯 HEROKU PRIORITY: Matching with interests:", {
            ...
            console.log("🎯 HEROKU PRIORITY: SUCCESS - Found tags:", suggestedAffinityTags.length);
          }
        } catch (affinityError) {
          console.error("❌ HEROKU PRIORITY: Affinity error:", affinityError);
        }

        // Generate other components
        const enhancedComments = await enhanceInteractionComments(finalTranscript, extractedInfo, userId);
        const qualityAssessment = await assessInteractionQuality(finalTranscript, extractedInfo);

        console.log("✅ HEROKU PRIORITY: Process completed - affinity tags:", suggestedAffinityTags.length);
        ...
        res.json({
          ...
          conciseSummary: extractedInfo.conciseSummary || '',
          enhancedComments,
          qualityAssessment
        });

      } catch (error) {
        console.error("❌ HEROKU PRIORITY: Voice processing error:", error);
        res.status(500).json({ 
